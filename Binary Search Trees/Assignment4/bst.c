...
int size_helper(struct bst_node *root){
	if(root==NULL){
		return 0;
	}
	int leftsize = size_helper(root->left);
...
int bst_path_sum(int sum, struct bst* bst) {
  ...
  return sum_helper(0,sum,bst->root);
}

int sum_helper(int newsum,int sum,struct bst_node* root){
	if(root==NULL){
		return 0;
	}
	...
	int left = sum_helper(newsum,sum,root->left);
	int right = sum_helper(newsum,sum,root->right);
	return left || right ;
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
	...
	iter->root = bst->root;
	iter->stack = stack;
	push_leftnode(iter->stack,bst->root);
...
int bst_iterator_next(struct bst_iterator* iter) {
	...
	push_leftnode(iter->stack,current->right);
  return current->val;
}

void push_leftnode(struct stack* stack, struct bst_node* root){
	while(root){
