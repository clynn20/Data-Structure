...
struct dynarray* create_product_array(int num_products, char** names, int* inventory, float* prices) {
	struct dynarray *item_arr = dynarray_create();
	for(int i=0;i<num_products;i++){
		dynarray_insert(item_arr,i,create_product(names[i],inventory[i],prices[i]));	
	}
  return item_arr;
}
...
void free_product_array(struct dynarray* products) {
	...
	for(int i=0;i<length;i++){
		struct product* item = (struct product*)(dynarray_get(products,i));
		free_product(item);
	}
...
struct product* find_max_investment(struct dynarray* products) {
	...
	for(int i=0;i<length;i++){
		struct product* item = (struct product*)(dynarray_get(products,i));
		if( (item->price * item->inventory) > max_invest){
			max_invest = item->price * item->inventory;
			max_index = i;
		}
	}
  return (struct product*)(dynarray_get(products,max_index));
...
void swap(struct product** x,struct product** y){
	struct product temp = **x;
	**x = **y;
	**y = temp;
}
...
void sort_by_inventory(struct dynarray* products) {
	...
	for(int i=0;i<length-1 ;i++){
		for(int j=0;j<length-i-1 ;j++){
			struct product* item = (struct product*)(dynarray_get(products,j));
			struct product* item_next = (struct product*)(dynarray_get(products,j+1));
			if(item->inventory > item_next->inventory){
				swap(&item, &item_next);
			}
		}
	}	
